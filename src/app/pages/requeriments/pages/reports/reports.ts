...
export class ReportsComponent implements OnInit, AfterViewInit {
  ...
  get pendientes()  { return this.filtered.filter(r => r.status === 'PENDING').length; }
  get enProceso()   { return this.filtered.filter(r => r.status === 'IN_PROGRESS').length; }
  get completados() { return this.filtered.filter(r => r.status === 'COMPLETED').length; }
  get cancelados()  { return this.filtered.filter(r => r.status === 'REJECTED').length; }
  ...
  areaData() {
    ...
    return {
      ...
      colors: ['#0A142D', '#CC521B', '#90574D', '#94B6EF', '#CCBBA7',
               '#2e7d32', '#1565c0', '#f57f17', '#6a1b9a', '#00838f'],
    };
  }
  ...
  priorityData() {
    return {
      ...
      values: ['BAJA', 'MEDIA', 'ALTA', 'CRITICA']
        .map(p => this.filtered.filter(r => r.priority === p).length),
  ...
  private drawBars(canvas: HTMLCanvasElement | undefined, data: any): void {
    ...
    data.values.forEach((v: number, i: number) => {
      ...
      const label = data.labels[i].length > 8
        ? data.labels[i].slice(0, 7) + '…'
        : data.labels[i];
  ...
  // ── Exports ──────────────────────────
  downloadCSV(): void {
    const headers = ['#', 'Área', 'Tipo', 'Título', 'Descripción',
                     'Estado', 'Prioridad', 'Solicitante', 'Fecha'];
    const rows = this.filtered.map(r => [
      r.id, r.areaName, r.typeName, r.title,
